    ...
    \${clinicSettings?.prescription_header_img && clinicSettings?.print_mode !== 'preprinted' ? '<div style="width:100%;margin-bottom:0"><img src="'+clinicSettings.prescription_header_img+'" style="width:100%;display:block;max-height:150px;object-fit:cover" alt="Header"/></div>' : '<div class="header"><div><div class="clinic-name">🏥 '+clinicName+'</div><div class="clinic-sub">'+clinicAddress+'</div><div class="clinic-sub" style="margin-top:4px">📞 '+clinicPhone+' · 👨‍⚕️ '+doctorName+'</div></div><div><div class="rx-badge">'+rx.id+'</div><div style="color:rgba(255,255,255,0.5);font-size:11px;text-align:right;margin-top:6px">'+formatUSDate(rx.date)+'</div></div></div>'}
      ...
      ${labQrToken?`<div class="section-title" style="margin-top:10px">🔬 Lab Results Upload (QR)</div><div style="display:flex;gap:14px;align-items:flex-start;border:1px solid #e2e8f0;border-radius:8px;padding:10px;margin-bottom:8px"><img src="https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=${encodeURIComponent(window.location.origin+'/lab-upload/'+labQrToken)}" width="90" height="90" style="border:1px solid #f1f5f9;border-radius:6px;flex-shrink:0"/><div style="font-size:10px;color:#0a1628"><strong style="font-size:11px">Scan to upload results</strong><br/><span style="color:#64748b">Lab/radiology department should scan this QR code to upload test results directly to the patient record.</span><br/><span style="color:#94a3b8;font-size:9px;margin-top:4px;display:block">Expires: ${labQrExpiry ? new Date(labQrExpiry).toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'}) : ''}</span></div></div>`:''}
        ...
        <div>
          ...
          \${clinicSettings?.doctor_signature_url ? '<img src="'+clinicSettings.doctor_signature_url+'" style="height:40px;margin-bottom:4px;display:block"/>' : ''}
          <div style="border-top:1px solid #374151;width:150px;padding-top:3px;font-size:9px;color:#6b7280;text-align:center">\${clinicSettings?.doctor_name||doctorName}<br>Signature & Stamp</div>
        </div>
        <div></div>
      </div>
      \${clinicSettings?.prescription_footer_img && clinicSettings?.print_mode !== 'preprinted' ? '<div style="width:100%;margin-top:8px"><img src="'+clinicSettings.prescription_footer_img+'" style="width:100%;display:block;max-height:80px;object-fit:cover" alt="Footer"/></div>' : ''}
    </div>
  </div></body></html>`;
...
export default function PrescriptionClient({
  data, clinicName, doctorName, clinicPhone, clinicAddress, clinicSettings
}: {
  data: Appointment[];
  clinicName: string; doctorName: string; clinicSettings?: any;
