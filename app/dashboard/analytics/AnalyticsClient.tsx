...
export default function AnalyticsClient({ data, stats, ...rest }: Props) {
  ...
  const exportPDF = () => {
    ...
    const monthRows = displayMonthly.map(m=>`<tr><td>${m.month}</td><td>${m.total}</td><td style="color:#1a7f5e">${m.confirmed}</td><td style="color:#c53030">${m.cancelled}</td><td style="color:#b47a00">${m.rescheduled}</td><td>${m.total?Math.round(m.confirmed/m.total*100):0}%</td></tr>`).join('');
    ...
    table{width:100%;border-collapse:collapse;margin-top:4px}th{background:#0a1628;color:#fff;padding:7px 12px;text-align:left;font-size:11px}
    ...
    <table><thead><tr><th>Month</th><th>Total</th><th>Confirmed</th><th>Cancelled</th><th>Rescheduled</th><th>Rate</th></tr></thead><tbody>${monthRows}</tbody></table>
    ...
    <div class="grid4">
      ...
      <div class="card"><div class="val" style="color:#0369a1">PKR ${consultInvoices.reduce((s,i)=>s+i.paid,0).toLocaleString()}</div><div class="lbl">Consultation Revenue</div></div>
      <div class="card"><div class="val" style="color:#6d28d9">PKR ${procedureInvoices.reduce((s,i)=>s+i.paid,0).toLocaleString()}</div><div class="lbl">Procedure Revenue</div></div>
    </div>
    <div class="footer">MediPlex Pediatric Clinic · Confidential · ${new Date().toLocaleDateString()}</div>
    </body></html>`;
    const w=window.open('','_blank'); if(!w){toast.error('Allow popups');return;}
