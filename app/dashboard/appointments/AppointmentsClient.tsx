...
export default function AppointmentsClient({ data }: { data: Appointment[] }) {
  ...
  const filtered = useMemo(() => filterAppointments(data, { status, visitType, search, dateFrom, dateTo }), [data, status, visitType, search, dateFrom, dateTo]);
  ...
  const clearFilters = () => {
    setStatus('all'); setVisitType('all'); setSearch(''); setDateFrom(''); setDateTo(''); setPage(1);
  };

  const hasFilters = status !== 'all' || visitType !== 'all' || search || dateFrom || dateTo;

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="text-[13px] text-gray-500">
          Showing <span className="font-medium text-navy">{filtered.length}</span> of <span className="font-medium text-navy">{data.length}</span> appointments
        </div>
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th>#</th><th>Patient</th><th>Age</th><th>Contact</th>
                <th>Appointment</th><th>Time</th><th>Reason</th><th>Visit Type</th><th>Status</th><th>Notes</th>
              </tr>
            </thead>
            <tbody>
              {slice.length === 0 && (
                ...
                <tr key={a.id} className={selected.includes(a.id) ? 'bg-amber-50/40' : ''}>
                  ...
                  <td className="max-w-[140px]">
                    {a.reschedulingReason && (
                      <div className="text-[11px] text-gray-400 truncate" title={a.reschedulingReason}>↻ {a.reschedulingReason}</div>
                    )}
                  </td>
                </tr>
              ))}
