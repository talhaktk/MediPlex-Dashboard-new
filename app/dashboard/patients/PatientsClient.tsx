                      ...
                      <div key={s.id||i} className="rounded-xl overflow-hidden" style={{border:`1px solid ${color}33`}}>
                        <div className="flex items-center justify-between px-4 py-2.5" style={{background:`${color}11`}}>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[11px] text-gray-400">{s.generated_at ? new Date(s.generated_at).toLocaleDateString() : ''}</span>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <button onClick={()=>{navigator.clipboard.writeText(s.output||'');toast.success('Copied!');}} className="text-[10px] px-2 py-1 rounded-lg" style={{background:'rgba(0,0,0,0.06)',color:'#6b7280'}}>Copy</button>
                            <button onClick={()=>setSelectedScribe(s)} className="text-[10px] px-2 py-1 rounded-full font-medium" style={{background:`${color}22`,color}}>View Full →</button>
                          </div>
                        </div>
                        <div className="px-4 py-3 cursor-pointer" onClick={()=>setSelectedScribe(s)}>
                          {lines.slice(0,4).map((line:string,j:number)=>{
              ...
              {activeTab==='procedures' && (
                <div className="p-5 space-y-4">
                  ...
                  {showProcedureForm && (()=>{
                    ...
                    return (<div className="rounded-xl p-4 space-y-4" style={{background:'rgba(201,168,76,0.06)',border:'1px solid rgba(201,168,76,0.25)'}}>
                      <div className="flex items-center justify-between">
                  ...
                  {procedures.length===0&&!showProcedureForm?<div className="text-center py-8 text-gray-400 text-[13px]">No procedures recorded</div>
                  :procedures.map((p,i)=>(
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0" style={{borderColor:'rgba(255,255,255,0.08)',background:`${SCRIBE_COLORS[selectedScribe.mode]||'#6b7280'}22`}}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={()=>{
                  ...
                  w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${selectedScribe.mode} - ${selected?.name}</title><style>body{font-family:Arial,sans-serif;padding:30px;max-width:800px;margin:0 auto;color:#0a1628}.hdr{background:#0a1628;color:white;padding:16px 20px;border-radius:8px 8px 0 0;display:flex;justify-content:space-between}.section{font-weight:700;font-size:12px;color:${color};margin-top:16px;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid ${color}33;padding-bottom:4px}.line{font-size:13px;margin:2px 0;line-height:1.6}.tbl{font-size:11px;font-family:monospace;color:#555}.allergy{background:#fff7ed;border:1px solid #fed7aa;padding:8px 14px;border-radius:6px;margin-bottom:16px;font-size:12px}@media print{body{padding:10px}}</style></head><body><div class="hdr"><div style="font-size:16px;font-weight:700">🏥 MediPlex Pediatric Centre</div><div style="font-size:12px;opacity:0.7">${selectedScribe.mode==='soap'?'SOAP Note':selectedScribe.mode==='discharge'?'Discharge Summary':'Referral Letter'}</div></div><div style="border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;padding:20px"><div class="allergy">Patient: <strong>${selected?.name}</strong> · Age: ${selected?.age} · MR#: ${selected?.mrNumber||'-'} · Date: ${selectedScribe.generated_at?new Date(selectedScribe.generated_at).toLocaleDateString():''}</div>${(selectedScribe.output||'').split('
').map((line:string)=>line.startsWith('**')&&line.endsWith('**')?`<div class="section">${line.replace(/\*\*/g,'')}</div>`:line.startsWith('|')?`<div class="tbl">${line}</div>`:line.startsWith('- ')||line.startsWith('• ')?`<div class="line" style="margin-left:16px">${line}</div>`:line.includes('⚠️')?`<div class="line" style="color:#ea580c;font-weight:600">${line}</div>`:`<div class="line">${line}</div>`).join('')}</div></body></html>`);
                  w.document.close();setTimeout(()=>w.print(),500);
                }} className="text-xs px-3 py-1.5 rounded-lg" style={{background:'rgba(255,255,255,0.08)',color:'rgba(255,255,255,0.6)'}}>Print</button>
