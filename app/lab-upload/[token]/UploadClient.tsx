...
import { Upload, CheckCircle, AlertCircle, FileText, X, Loader2, Phone } from 'lucide-react';

interface OrderInfo {
  orderId: string; patientName: string; orderType: 'lab' | 'radiology';
  tests: { name: string; category: string }[]; clinicalNotes: string;
  status: string; expiresAt: string; hasPhone: boolean; phoneMask: string | null;
}
  ...
  if (step === 'done') return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'linear-gradient(135deg,#0a1628 0%,#142240 100%)' }}>
      <div className="text-center max-w-sm">
        ...
        <p className="text-white/50 text-sm mb-4">The doctor has been notified and results are attached to the patient record.</p>
        {uploadRes?.hasAbnormal && (
          <div className="rounded-xl p-3 mb-4 text-sm"
            style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: '#fca5a5' }}>
            ⚠️ Abnormal values detected. The doctor will review them.
          </div>
        )}
        <div className="flex gap-3 justify-center text-white/40 text-xs">
          {uploadRes?.filesUploaded ? <span>✓ {uploadRes.filesUploaded} file(s)</span> : null}
          {uploadRes?.resultValues  ? <span>✓ {uploadRes.resultValues} test values</span> : null}
        </div>
      ...
      <div className="max-w-lg mx-auto">
        ...
        {order && (
          <div className="rounded-2xl p-5 mb-5" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(201,168,76,0.2)' }}>
            ...
            {order.clinicalNotes && <p className="text-white/30 text-xs mt-3 italic">Note: {order.clinicalNotes}</p>}
          </div>
        )}
        ...
        <div className="rounded-2xl p-6" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-white/40 text-sm">
                  Enter the patient's registered mobile number
                  {order?.phoneMask ? ` (ending in ${order.phoneMask})` : ''}.
                </p>
              </div>
              <div>
                <label className="text-[11px] text-white/40 uppercase tracking-widest block mb-1.5">Mobile / WhatsApp Number</label>
                <div className="relative">
                  <Phone size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30" />
                  <input type="tel" value={phone} onChange={e => setPhone(e.target.value)}
                    placeholder="+92 300 0000000" className={inp + " pl-9"} style={iStyle}
                    onKeyDown={e => e.key === 'Enter' && handleVerify()} />
                </div>
              </div>
              {error && <p className="text-red-400 text-xs">{error}</p>}
              <button onClick={handleVerify} disabled={!phone.trim()}
          ...
          {step === 'upload' && (
            <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    {results.map((r, i) => (
                      <div key={i} className="flex items-center gap-2">
                        <span className="text-white/70 text-xs w-36 flex-shrink-0 truncate">{r.testName}</span>
                        <input value={r.value}
                        ...
                        <input value={r.unit}
                          onChange={e => setResults(prev => prev.map((x, j) => j === i ? { ...x, unit: e.target.value } : x))}
              ...
              <div>
                <label className="text-[11px] text-white/40 uppercase tracking-widest block mb-1.5">Attach Files</label>
                <div className="border-2 border-dashed rounded-xl p-5 text-center cursor-pointer"
                  ...
                  <p className="text-white/40 text-sm">Click to attach PDF or images</p>
                  <input ref={fileRef} type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.tiff"
                    onChange={e => setFiles(prev => [...prev, ...Array.from(e.target.files || [])])} className="hidden" />
                </div>
                {files.length > 0 && (
                  <div className="mt-2 space-y-1">
                    {files.map((f, i) => (
                      <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.05)' }}>
                        ...
                        <button onClick={() => setFiles(prev => prev.filter((_, j) => j !== i))} className="text-white/30 hover:text-red-400"><X size={12} /></button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <button onClick={handleSubmit}
                disabled={submitting || (files.length === 0 && results.every(r => !r.value.trim()) && !reportText.trim())}
