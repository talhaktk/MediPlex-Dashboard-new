  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="text-[13px] font-semibold text-navy">Lab & Radiology Orders</div>
        <div className="flex gap-2">
          <button onClick={load} className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600" style={{background:'#f1f5f9'}}>
            <RefreshCw size={12}/>
          </button>
      ...
      {showForm==='lab' && (
        <div className="rounded-xl p-4 space-y-3" style={{background:'rgba(26,127,94,0.08)',border:'1px solid rgba(26,127,94,0.25)'}}>
              ...
              {selectedTests.map((t,i)=>(
                <div key={i} className="flex items-center gap-2">
                  ...
                  <select value={t.urgency} onChange={e=>setSelectedTests(prev=>prev.map((x,j)=>j===i?{...x,urgency:e.target.value}:x))}
                    className="rounded px-2 py-1 text-[10px] outline-none" style={{background:'#f1f5f9',border:'1px solid #e2e8f0',color:'#0a1628'}}>
                    {URGENCY_OPTS.map(u=><option key={u} value={u} style={{background:'#f1f5f9'}}>{u}</option>)}
                  </select>
                  <button onClick={()=>setSelectedTests(prev=>prev.filter((_,j)=>j!==i))} className="text-gray-400 hover:text-red-400"><X size={12}/></button>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="flex gap-2">
            ...
            <button onClick={()=>{setShowForm(null);setSelectedTests([]);}} className="px-3 py-2 rounded-lg text-[11px] text-gray-500" style={{background:'rgba(255,255,255,0.05)'}}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {showForm==='radiology' && (
        <div className="rounded-xl p-4 space-y-3" style={{background:'rgba(59,130,246,0.08)',border:'1px solid rgba(59,130,246,0.25)'}}>
          <div className="text-[12px] font-semibold text-blue-600">New Radiology Order</div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-widest block mb-1">Type</label>
              <select value={radioType} onChange={e=>setRadioType(e.target.value)} className="w-full rounded-lg px-3 py-2 text-[12px] outline-none" style={{background:'#f1f5f9',border:'1px solid #e2e8f0',color:'#0a1628'}}>
                {RADIOLOGY_TYPES.map(t=><option key={t} value={t} style={{background:'#f1f5f9'}}>{t}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-widest block mb-1">Urgency</label>
              <select value={radioUrgency} onChange={e=>setRadioUrgency(e.target.value)} className="w-full rounded-lg px-3 py-2 text-[12px] outline-none" style={{background:'#f1f5f9',border:'1px solid #e2e8f0',color:'#0a1628'}}>
                {URGENCY_OPTS.map(u=><option key={u} value={u} style={{background:'#f1f5f9'}}>{u}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-widest block mb-1">Body Part / Region *</label>
              <input value={bodyPart} onChange={e=>setBodyPart(e.target.value)} placeholder="e.g. Chest, Right Knee..." className="w-full rounded-lg px-3 py-2 text-[12px] outline-none" style={{background:'#f1f5f9',border:'1px solid #e2e8f0',color:'#0a1628'}} onFocus={e=>(e.target.style.borderColor='rgba(201,168,76,0.5)')} onBlur={e=>(e.target.style.borderColor='rgba(255,255,255,0.1)')}/>
            </div>
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-widest block mb-1">View / Position</label>
            ...
            <div>
              <label className="text-[10px] text-gray-500 uppercase tracking-widest block mb-1">Contrast</label>
              <select value={contrast} onChange={e=>setContrast(e.target.value)} className="w-full rounded-lg px-3 py-2 text-[12px] outline-none" style={{background:'#f1f5f9',border:'1px solid #e2e8f0',color:'#0a1628'}}>
                {['Without','With','With & Without'].map(c=><option key={c} value={c} style={{background:'#f1f5f9'}}>{c}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={()=>setShowForm(null)} className="px-3 py-2 rounded-lg text-[11px] text-gray-500" style={{background:'rgba(255,255,255,0.05)'}}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-gray-300 text-[12px]">No lab orders yet</div>
      ) : (
                  ...
                  <div className="px-4 pb-4 space-y-3" style={{borderTop:'1px solid rgba(255,255,255,0.05)'}}>
                    ...
                    {orderResults.length > 0 && (
                      <div className="space-y-2">
                        <p className="text-[10px] text-gray-400 uppercase tracking-widest">Results from {orderResults[0]?.lab_name||'Lab'}</p>
                        {orderResults.map(r=>(
                          <div key={r.id} className="rounded-lg p-3" style={{background:'rgba(26,127,94,0.08)',border:'1px solid rgba(26,127,94,0.2)'}}>
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-[10px] text-gray-400">{r.visit_date||r.uploaded_at?.slice(0,10)}</span>
                            </div>
                            {r.notes && <p className="text-[11px] text-gray-500 mb-2">{r.notes}</p>}
                            {/* Files */}
              ...
              <div className="space-y-2">
                {results.filter(r=>r.order_id==null||r.order_id===undefined).map(r=>(
                  <div key={r.id} className="rounded-xl p-3" style={{background:'#f9fafb',border:'1px solid #e2e8f0'}}>
                    ...
                    {r.lab_name && <p className="text-[10px] text-gray-400 mb-2">{r.lab_name}</p>}
                    {(r.file_urls||[]).length>0 && (
                      <div className="flex flex-wrap gap-2">
                        {(r.file_urls as string[]).map((url:string,i:number)=>(
                          isImage(url) ? (
                            <button key={i} onClick={()=>setLightbox(url)} className="w-14 h-14 rounded-lg overflow-hidden" style={{border:'1px solid #e2e8f0'}}>
                              <img src={url} alt="result" className="w-full h-full object-cover"/>
                            </button>
                          ) : (
                            <a key={i} href={url} target="_blank" rel="noopener noreferrer"
